...
type SeedEchoMote = {
  id: string; x: number; y: number; seedRef: string;
  sprite: Phaser.GameObjects.Arc; halo: Phaser.GameObjects.Arc; touched: boolean;
};
...
function buildMap(region: ImaginalRegion): number[][] {
  ...
  for (let y = 0; y < MAP_H; y++) {
    ...
    for (let x = 0; x < MAP_W; x++) {
      ...
      else if (region === "corridor" && ((x === 2 || x === 7) && (y === 3 || y === 6))) row.push(T.MOON_PILLAR);
...
export class ImaginalRealmScene extends Phaser.Scene {
  ...
  init(data: { save: SaveSlot }) {
    ...
    if (!this.save.region) { this.save.region = "pools"; writeSave(this.save); }
  }
  ...
  create() {
    ...
    this.daimonMark = new GBCText(this, GBC_W - 28, 2, bondLabel, { color: COLOR.textAccent, depth: 220, scrollFactor: 0 });
    ...
    this.companion = new SorynCompanion(
      ...
      this.rowan,
      () => this.companionLines(),
      ["BREATHE.", "I AM HERE.", "THE MIRROR LIES KINDLY.", "YOU ARE DOING WELL.", "WITNESS WHEN UNSURE."],
    );

    this.add.rectangle(0, 13, GBC_W, 9, 0x0a0e1a, 0.85).setOrigin(0, 0).setDepth(199);
    this.titleText = new GBCText(this, 4, 14, REGION_TITLE[this.region], { color: COLOR.textAccent, depth: 200 });
    this.add.rectangle(0, GBC_H - 11, GBC_W, 11, 0x0a0e1a, 0.85).setOrigin(0, 0).setScrollFactor(0).setDepth(199);
    this.hint = new GBCText(this, 4, GBC_H - 9, "WALK", { color: COLOR.textDim, depth: 200, scrollFactor: 0 });
    ...
    if (!this.save.flags.imaginal_intro) {
      ...
      this.time.delayedCall(400, () => runDialog(this, [
        ...
        { who: "Soryn", text: "Walk south to leave each region. I will be a half-step behind." },
      ], () => { this.dialogActive = false; this.loadRegion(this.region); }));
    } else {
  ...
  private loadRegion(region: ImaginalRegion) {
    ...
    if (region === "pools") {
      spawnMotes(this, { count: 16, color: 0x88c0e8, alpha: 0.55, driftY: -0.004, driftX: 0.003, depth: 30 });
      // Pool reflections — translucent puddles
      [{ x: 32, y: 80 }, { x: 128, y: 80 }, { x: 80, y: 96 }].forEach(p => {
        const pool = this.add.ellipse(p.x, p.y, 22, 6, 0x88c0e8, 0.35).setDepth(1);
        this.regionRoot.add(pool);
      });
    } else if (region === "field") {
      spawnMotes(this, { count: 18, color: 0xffe098, alpha: 0.5, driftY: -0.008, driftX: 0.004, depth: 30 });
      spawnMotes(this, { count: 6, color: 0xa8c8e8, alpha: 0.4, driftY: 0.005, driftX: -0.003, depth: 30 });
      this.spawnSeedEchoes();
    } else {
      spawnMotes(this, { count: 8, color: 0x5a78b8, alpha: 0.4, driftY: -0.002, driftX: 0, depth: 30 });
    ...
    for (const def of defineLayout().filter(k => k.region === region)) {
      ...
      const sprite = this.add.image(def.x, def.y, "gbc_tiles", cleared ? TILE_INDEX.MIRROR_CLEARED : TILE_INDEX.MIRROR_FRAME);
      ...
      if (!cleared) {
        ...
        this.tweens.add({ targets: sprite, y: def.y - 1, duration: 1400, yoyo: true, repeat: -1, ease: "Sine.inOut" });
      }
    ...
    if (region === "pools") { this.rowan.setPosition(24, 56); }
    else { this.rowan.setPosition(80, 32); } // entered from north
    ...
    if (!this.save.flags[introFlag]) {
      ...
      writeSave(this.save);
      this.dialogActive = true;
      this.time.delayedCall(500, () => runDialog(this, this.regionIntro(region), () => { this.dialogActive = false; }));
    }
  ...
  private spawnSeedEchoes() {
    const seeds = ["seed_call", "seed_window", "seed_kettle", "seed_coat", "seed_mirror"]
      .filter(s => this.save.seeds[s] && !this.save.seedEchoes[s]);
    const positions = [
      { x: 60, y: 80 }, { x: 100, y: 80 }, { x: 30, y: 96 }, { x: 130, y: 96 }, { x: 80, y: 100 },
    ];
    seeds.forEach((seed, i) => {
      ...
      this.tweens.add({ targets: halo, scale: 1.6, alpha: 0.1, duration: 1100, yoyo: true, repeat: -1 });
      this.tweens.add({ targets: sprite, y: p.y - 2, duration: 900, yoyo: true, repeat: -1, ease: "Sine.inOut" });
      this.regionRoot.add(halo); this.regionRoot.add(sprite);
      this.seedEchoes.push({ id: `${seed}_echo`, x: p.x, y: p.y, seedRef: seed, sprite, halo, touched: false });
    });
  }

  private regionIntro(region: ImaginalRegion): { who: string; text: string }[] {
    if (region === "pools") return [
      { who: "Soryn", text: "The Pools. Standing silver-water. Things that mimic move slowly." },
      { who: "Soryn", text: "Two knots here. The one west copies you. The one east speaks back." },
    ];
    if (region === "field") return [
      { who: "Soryn", text: "The Field. Suspended afternoon-light. Time is paused here, kindly." },
      { who: "Soryn", text: "The little gold motes are echoes of what you touched in your last day." },
      { who: "Soryn", text: "Walk through them. They will speak." },
    ];
    return [
      { who: "Soryn", text: "The Corridor. Narrow. A wished-self waits — optional." },
      { who: "Soryn", text: "The door at the south leads to the Curated Self. I cannot enter it with you." },
    ];
  }
  ...
  update(_t: number, dt: number) {
    ...
    for (const m of this.seedEchoes) {
      ...
      const sdx = this.rowan.x - m.x, sdy = this.rowan.y - m.y;
      if (sdx * sdx + sdy * sdy < 7 * 7) this.touchSeedEcho(m);
    }

    // Edge transitions
    if (this.region === "pools" && this.rowan.y > GBC_H - 20 && this.allowExit("pools")) this.transitionTo("field");
    else if (this.region === "field" && this.rowan.y > GBC_H - 20 && this.allowExit("field")) this.transitionTo("corridor");
    ...
    if (near && this.dist(near) < 16 * 16) {
      ...
      if (this.region === "corridor") {
        ...
        if (cleared >= 4 && this.rowan.y > GBC_H - 24) this.hint.setText("A: ENTER THE CURATED SELF");
        else this.hint.setText(`KNOTS QUIETED ${cleared}/4 NEEDED`);
      } else {
        this.hint.setText(this.region === "pools" ? "WALK SOUTH TO THE FIELD" : "WALK SOUTH TO THE CORRIDOR");
      }
    }
  }
  ...
  private touchSeedEcho(m: SeedEchoMote) {
    ...
    const t = new GBCText(this, m.x - 28, m.y - 12, lines[m.seedRef] ?? "AN ECHO.", { color: COLOR.textGold, depth: 220 });
    this.tweens.add({ targets: t.obj, alpha: 0, y: m.y - 28, duration: 1800, onComplete: () => t.destroy() });
    this.tweens.add({ targets: m.sprite, scale: 3, alpha: 0, duration: 600, onComplete: () => m.sprite.destroy() });
    this.tweens.add({ targets: m.halo, scale: 3, alpha: 0, duration: 600, onComplete: () => m.halo.destroy() });
    if (extraShard) {
      this.cameras.main.flash(160, 255, 224, 152);
      const s = new GBCText(this, m.x - 28, m.y - 22, "MEMORY SHARD +1", { color: COLOR.textGold, depth: 221 });
      this.tweens.add({ targets: s.obj, alpha: 0, y: m.y - 36, duration: 1600, onComplete: () => s.destroy() });
    }
  }
  ...
  // ============================================================================
  private dist(k: Knot) {
    const dx = this.rowan.x - k.x, dy = this.rowan.y - k.y;
    return dx * dx + dy * dy;
  }
  private nearestKnot(): Knot | null {
    let best: Knot | null = null; let bd = Infinity;
    for (const k of this.knots) { const d = this.dist(k); if (d < bd) { bd = d; best = k; } }
    return best;
  }
  ...
  private launchKnot(k: Knot) {
    ...
    dispatchKnot(this, k.kind, this.rowan, { x: k.x, y: k.y }, this.save, (r: KnotResult) => {
      ...
      if (r.flags) r.flags.forEach(f => { this.save.flags[f] = true; });
      writeSave(this.save);
      this.events.emit("stats-changed");
      k.sprite.setFrame(TILE_INDEX.MIRROR_CLEARED);
      if (k.glow) { k.glow.destroy(); k.glow = undefined; }
    });
  }
