...
export function runReflectionKnot(
  ...
  const label = new GBCText(
    scene,
    8,
    GBC_H - 28,
    "STAND STILL. WHEN YOUR REFLECTION OVERLAPS, A.",
    {
      ...
      [
        { who: "Soryn", text: "You stood and saw it. The mimic had no inside." },
        {
          ...
          text: seedAware(
            save,
            "seed_window",
            "It only knew the wave you did not return.",
            "It only knew the shape of you.",
          ),
        },
      ],
...
// ============================================================================
export function runEchoKnot(scene: Phaser.Scene, save: SaveSlot, onDone: (r: KnotResult) => void) {
  const seedCall = save.seeds.seed_call;
  const seedKettle = save.seeds.seed_kettle;
  const original = seedCall
    ? "I should have called Mara."
    : seedKettle
      ? "I always pour two cups."
      : "I am tired of being unseen.";
  const garbled: import("../../inquiry").InquiryOption[] = [
    {
      choice: "ask",
      label: "I SHOULD CALL TOMORROW.",
      reply: "Close. But too late was always the rhythm.",
    },
    { choice: "ask", label: original.toUpperCase(), reply: "Yes. The echo answers, kindly." },
    {
      choice: "observe",
      label: "I AM FINE. I AM FINE.",
      reply: "That was the surface. Not the line beneath.",
    },
    {
      choice: "silent",
      label: "(SAY NOTHING)",
      reply: "Silence is also an echo. Quieter, not gone.",
    },
  ];
  // Shuffle
...
export function runGlitterKnot(
  ...
  if (shuffled.every((s, i) => s === correct[i]))
    [shuffled[0], shuffled[1]] = [shuffled[1], shuffled[0]];

  const box = drawGBCBox(scene, 4, GBC_H - 60, GBC_W - 8, 56, 250);
  const title = new GBCText(scene, 8, GBC_H - 56, "ORDER THE AFTERNOON.", {
    color: COLOR.textAccent,
    depth: 251,
  });
  const hint = new GBCText(scene, 8, GBC_H - 12, "L/R: SWAP  A: CONFIRM", {
    color: COLOR.textDim,
    depth: 251,
  });
  let cursor = 0;
  const items: GBCText[] = shuffled.map(
    (s, i) => new GBCText(scene, 16, GBC_H - 46 + i * 8, s, { color: COLOR.textLight, depth: 251 }),
  );
  const cur = new GBCText(scene, 8, GBC_H - 46, "▶", { color: COLOR.textGold, depth: 251 });

  const refresh = () => {
    items.forEach((t, i) => {
      t.setText(shuffled[i]);
        ...
        [
          { who: "Soryn", text: "The whole afternoon. Even the dull parts." },
          {
            ...
            text: seedAware(
              save,
              "seed_kettle",
              "The second cup was for someone who knew you. You knew it.",
              "Memory in pieces is still memory.",
            ),
          },
        ],
        () =>
  ...
  runInquiry(
    ...
    [
      ...
      { choice: "ask", label: "ADDRESS", reply: "It answers in your voice. The light grows." },
      {
        choice: "silent",
        label: "REMEMBER",
        reply: "You wrap the lie in memory. The light grows.",
      },
          ...
          [
            { who: "Soryn", text: "Some lights are not meant to be kept." },
            {
              ...
              text: seedAware(
                save,
                "seed_coat",
                "The coat by the door. The unfinished things. They were not yours to finish.",
                "The kindest lies are the hardest to put down.",
              ),
            },
          ],
          () =>
            ...
            runInquiry(
              ...
              [
                { choice: "confess", label: "RELEASE", reply: "Yes. Let it dim." },
                {
                  choice: "silent",
                  label: "KEEP IT",
                  reply: "Then it keeps you. We move on either way.",
                },
              ],
              (p2) => {
                scene.tweens.add({
...
export function runCrownKnot(scene: Phaser.Scene, save: SaveSlot, onDone: (r: KnotResult) => void) {
  ...
  runInquiry(
    scene,
    { who: "Crown", text: "AM I NOT WHO YOU WANTED TO BE?" },
    [
      {
        choice: "observe",
        label: "OBSERVE",
        reply: "It preens under your gaze. It grows brighter.",
      },
      { choice: "ask", label: "ADDRESS", reply: "It answers like a king. It grows brighter." },
      {
        choice: "confess",
        label: "WITNESS",
        reply: "You stand and see it whole. The crown is paper.",
      },
      {
        choice: "silent",
        label: "RELEASE",
        reply: "You let it stay. It will be smaller next time.",
      },
    ],
    (picked) => {
...
export function dispatchKnot(
  ...
  switch (kind) {
    case "reflection":
      return runReflectionKnot(scene, rowan, knotXY, save, onDone);
    case "echo":
      return runEchoKnot(scene, save, onDone);
    case "glitter":
      return runGlitterKnot(scene, save, onDone);
    case "lantern":
      return runLanternKnot(scene, save, onDone);
    case "crown":
      return runCrownKnot(scene, save, onDone);
  }
}
